...
function categoryLabel(code,lang){
  const ru={short:'Короткий username',english_word:'Английское слово',russian_word_translit:'Русское слово в латинице',other_language_word:'Слово на другом языке',first_name:'Имя',surname:'Фамилия',geography:'География',numeric:'Цифровой username',pattern:'Редкий паттерн',leetspeak:'Leetspeak',abbreviation:'Аббревиатура',random:'Случайная комбинация'};
  const en={short:'Short username',english_word:'English word',russian_word_translit:'Russian word in Latin script',other_language_word:'Other-language word',first_name:'First name',surname:'Surname',geography:'Geography',numeric:'Numeric username',pattern:'Rare pattern',leetspeak:'Leetspeak',abbreviation:'Abbreviation',random:'Random combination'};
...
function specialBrandResponse(username,platform,lang){
  ...
  return {username,platform,engineVersion:ENGINE_VERSION,priceMin:0,priceMax:0,openEnded:false,uncertain:true,specialCase:'global_brand',category:lang==='en'?'Special case — global brand':'Особый случай — глобальный бренд',categoryCode:'abbreviation',qualityScore:95,liquidity:'low',liquidityLabel:localizedLiquidity('low',lang),factors:lang==='en'?[{title:'Special case',text:'This is an exact match for a globally famous brand.'},{title:'Market',text:'It is not comparable with the ordinary free username market.'},{title:'Valuation',text:'A reliable normal-market price range would be misleading.'}]:[{title:'Особый случай',text:'Это точное совпадение со всемирно известным брендом.'},{title:'Рынок',text:'Такой username нельзя корректно сравнивать с обычным свободным рынком.'},{title:'Оценка',text:'Обычный ценовой диапазон здесь был бы вводящим в заблуждение.'}],disclaimer:lang==='en'?'Special-case classification: no ordinary market valuation is shown.':'Особый случай: обычная рыночная оценка не указывается.'};
}
...
function anchorResponse(username,platform,lang){
  ...
  const priceMin=anchor.min, priceMax=anchor.max, score=qualityFromAnchor(priceMin,priceMax,username), liquidity=liquidityFromAnchor(priceMin,priceMax), code=anchorCategoryCode(username);
  return {username,platform,engineVersion:ENGINE_VERSION,priceMin,priceMax,openEnded:Boolean(anchor.openEnded),uncertain:Boolean(anchor.uncertain),specialCase:'none',category:categoryLabel(code,lang),categoryCode:code,qualityScore:score,liquidity,liquidityLabel:localizedLiquidity(liquidity,lang),factors:lang==='en'?[{title:'Type',text:categoryLabel(code,'en')+'.'},{title:'Scarcity',text:`The ${[...username].length}-character format and composition are reflected in the estimate.`},{title:'Demand',text:liquidity==='high'?'Broad realistic buyer demand.':liquidity==='medium'?'Moderate realistic buyer demand.':'Narrow realistic buyer demand.'}]:[{title:'Тип',text:categoryLabel(code,'ru')+'.'},{title:'Редкость',text:`Длина ${[...username].length} символа и состав username учтены в оценке.`},{title:'Спрос',text:liquidity==='high'?'Широкий реальный спрос.':liquidity==='medium'?'Умеренный реальный спрос.':'Узкий реальный спрос.'}],disclaimer:lang==='en'?'Indicative SovaZone estimate, not a guaranteed sale price.':'Ориентировочная оценка SovaZone, не гарантия цены сделки.'};
}
...
function applyShortStructureGuardrails(parsed,username,lang){
  ...
  const hasSeparator=/[._]/.test(lower), code=parsed.categoryCode, structuralOnly=['short','random','pattern','numeric'].includes(code);
  ...
  if(len===3 && structuralOnly && (code==='random' || code==='short')){
    if(hasSeparator){ parsed.priceMin=floor; parsed.priceMax=Math.max(floor,Math.min(parsed.priceMax,2000)); }
  ...
  if(Array.isArray(parsed.factors) && parsed.factors.length){ parsed.factors[0]={title:lang==='en'?'Scarcity floor':'Минимум за редкость',text:lang==='en'?`${len}-character Instagram usernames keep a scarcity floor of $${floor.toLocaleString('en-US')}, including dot/underscore forms.`:`Instagram username длиной ${len} символа сохраняет минимум $${floor.toLocaleString('en-US')} за редкость, включая варианты с точкой или _.`}; }
  return parsed;
}
...
function applyLongSeparatorGuardrails(parsed,username){
  ...
  const originalMin=Number(parsed.priceMin)||0, originalMax=Number(parsed.priceMax)||0;
  parsed.priceMin=Math.min(100,niceRound(originalMin*0.015));
  parsed.priceMax=Math.min(300,Math.max(parsed.priceMin,niceRound(originalMax*0.02)));
...
function enforceRangeDiscipline(parsed){
  ...
  if(min>0){ const multiplier=parsed.uncertain?3:2; max=Math.min(max,min*multiplier); if(min>=40000 && max>min*1.6){ parsed.openEnded=true; parsed.priceMin=niceRound(min); parsed.priceMax=parsed.priceMin; return parsed; } }
...
function applyGuardrails(parsed,username,platform,lang){
  parsed.username=username; parsed.platform=platform; parsed.engineVersion=ENGINE_VERSION; parsed.specialCase=parsed.specialCase==='global_brand'?'global_brand':'none'; parsed.openEnded=Boolean(parsed.openEnded); parsed.uncertain=Boolean(parsed.uncertain);
  ...
  if(platform==='instagram'){ parsed=applyShortStructureGuardrails(parsed,username,lang); parsed=applyLongSeparatorGuardrails(parsed,username); parsed=applyPatternFivePlus(parsed,username); }
...
export default async function handler(req,res){
  ...
  const anchor=anchorResponse(username,platform,lang); if(anchor) return res.status(200).json(anchor);
  const cacheKey=`${ENGINE_VERSION}:${platform}:${lang}:${username.toLowerCase()}`;
  const cached=valuationCache.get(cacheKey); if(cached) return res.status(200).json(cached);
  ...
  const requestBody={model:process.env.VALUATION_MODEL||'gpt-5.6-luna',store:false,reasoning:{effort:'none'},temperature:0,instructions:CALIBRATION+'\n'+languageRules(lang),input:`Evaluate @${username} for ${platform.toUpperCase()}.\nDeterministic format signals: ${JSON.stringify(signals)}\nReturn the SovaZone valuation range and stable classification. Use openEnded=true only for $40k+ / $50k+ style cases. Use uncertain=true only when a materially wider range is genuinely necessary.`,max_output_tokens:700,text:{verbosity:'low',format:{type:'json_schema',name:'username_valuation_v30',strict:true,schema}}};
  try{
  ...
  }catch(e){ console.error('username-value v3 error',e); return res.status(500).json({error:lang==='en'?'Could not complete the estimate.':'Не удалось выполнить оценку.'}); }
}
