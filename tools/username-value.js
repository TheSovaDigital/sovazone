(function(){
  ...
  function cleanUsername(v){
    v=String(v||'').trim();
    v=v.replace(/^https?:\/\/(www\.)?(instagram\.com|tiktok\.com|t\.me|telegram\.me|fragment\.com\/username)\//i,'');
  ...
  function liquidityLabel(data){var v=String(data.liquidity||'low');if(v==='high')return t('Высокая ликвидность','High liquidity');if(v==='medium')return t('Средняя ликвидность','Medium liquidity');return t('Низкая ликвидность','Low liquidity');}
  function cacheKey(username){return ['sova-valuation',CACHE_VERSION,platform,lang,String(username||'').toLowerCase()].join(':');}
  function readCache(username){try{var raw=localStorage.getItem(cacheKey(username));if(!raw)return null;var data=JSON.parse(raw);if(!data||data.engineVersion!==CACHE_VERSION)return null;return data;}catch(e){return null;}}
  function writeCache(username,data){try{if(data&&data.engineVersion===CACHE_VERSION)localStorage.setItem(cacheKey(username),JSON.stringify(data));}catch(e){}}
  ...
  form.addEventListener('submit',async function(e){
    ...
    try{
      var res=await fetch(API_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:username,platform:platform,lang:lang,website:''})});
